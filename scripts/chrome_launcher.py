...
def launch_chrome(
    ...
    # On macOS, a direct second invocation of Chrome can be handed to the
    # already-running primary Chrome app and then exit immediately. That makes
    # a dedicated user-data-dir window appear to flash and leaves no CDP port.
    # `open -na` forces a separate app instance while retaining the same Chrome
    # arguments and profile isolation used by the publisher.
    if sys.platform == "darwin":
        app_bundle = chrome_path.split("/Contents/MacOS/", 1)[0]
        launch_cmd = ["open", "-na", app_bundle, "--args", *cmd[1:]]
    else:
        launch_cmd = cmd
