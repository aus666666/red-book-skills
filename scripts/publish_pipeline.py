...
def _select_topics(
    ...
    """Type each tag, confirming indeterminate timeouts against native anchors."""
    if not tags:
        return
    ...
    for index, tag in enumerate(tags):
        ...
        original_timeout = publisher.command_timeout_seconds
        publisher.command_timeout_seconds = max(original_timeout, 30.0)
        try:
            ...
            is_runtime_timeout = (
                "Timed out waiting for CDP response to Runtime.evaluate" in str(exc)
            )
            if not is_runtime_timeout:
                raise

            print(
                f"[pipeline] Topic selection response timed out for {tag}; "
                "reconnecting to reconcile native topic anchors."
            )
            publisher.disconnect()
            time.sleep(_jitter_seconds(1.0, timing_jitter, minimum_seconds=0.5))
            publisher.connect(
                target_url_prefix="https://creator.xiaohongshu.com/publish",
                reuse_existing_tab=True,
            )
            form = publisher._current_publish_form()
            if form.get("title") != expected_title:
                raise CDPError(
                    "Refusing topic-timeout recovery on the wrong form: "
                    f"expected {expected_title!r}, found {form.get('title')!r}."
                ) from exc

            def read_topic_state(focus_editor: bool = False):
                focus_literal = "true" if focus_editor else "false"
                return publisher._evaluate(
                    ...
                    (() => {{
                        ...
                        if (!editor) return {{ topics: [], suggestions: [] }};
                        if ({focus_literal}) {{
                            editor.focus();
                        ...
                        return {{
                            topics: Array.from(
                                editor.querySelectorAll('a.tiptap-topic[data-topic]')
                            ).map((node) => {{
                                try {{
                                    return JSON.parse(
                            ...
                            suggestions: Array.from(
                                document.querySelectorAll('span.suggestion')
            ...
            expected_prior_topics = [
                item.lstrip("#").strip()
                for item in tags[:index]
                if item.lstrip("#").strip()
            ]
            expected_topics = expected_prior_topics + [normalized_tag]
            expected_suggestion = [f"#{normalized_tag}"]
            topic_state = read_topic_state()

            if (
                topic_state.get("topics") == expected_prior_topics
                and topic_state.get("suggestions") == expected_suggestion
            ):
                # Re-read after focusing because the platform can finish the
                # pending suggestion asynchronously between CDP commands.
                topic_state = read_topic_state(focus_editor=True)
                if topic_state.get("topics") != expected_topics:
                    if not (
                        topic_state.get("topics") == expected_prior_topics
                        and topic_state.get("suggestions") == expected_suggestion
                    ):
                        raise CDPError(
                            f"Topic {tag} changed to an unsafe intermediate state "
                            f"during timeout recovery: {topic_state!r}."
                        ) from exc
                    common = {
                        "key": "Enter",
                        "code": "Enter",
                    ...
                    # The editor can acknowledge the key before it replaces the
                    # visible suggestion with a native topic anchor. Poll without
                    # sending another key; a second Enter can duplicate or corrupt
                    # the preserved form.
                    topic_commit_deadline = time.monotonic() + 8.0
                    while True:
                        time.sleep(
                            _jitter_seconds(
                                0.7, timing_jitter, minimum_seconds=0.4
                            )
                        )
                        topic_state = read_topic_state()
                        if topic_state.get("topics") == expected_topics:
                        ...
                        if not (
                            topic_state.get("topics") == expected_prior_topics
                            and topic_state.get("suggestions")
                            == expected_suggestion
                        ):
                            break
                        if time.monotonic() >= topic_commit_deadline:
                            break

            native_topics = topic_state.get("topics") or []
            if native_topics != expected_topics:
                raise CDPError(
                    f"Topic {tag} was not recovered as the exact ordered native "
                    "anchor set after Runtime.evaluate timed out; the preserved "
                    f"form requires narrow recovery. state={topic_state!r}"
                ) from exc
            result = {"ok": True, "selected": True, "recovered": True}
            print(
                f"[pipeline] Topic recovered from native anchor after timeout: {tag}"
            )
        finally:
            publisher.command_timeout_seconds = original_timeout
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help="Select a creator-center content declaration before publishing",
    )
    parser.add_argument(
        "--originality-declaration",
        action="store_true",
        default=False,
        help="Select and verify the creator-center originality declaration",
    )
    parser.add_argument(
        "--location",
        default=None,
        help="Select an exact creator-center location before publishing",
    )
    parser.add_argument(
        "--location-address",
        default=None,
        help="Require this exact address for the selected location",
    )
    ...
    try:
        ...
        if args.content_declaration:
            print(
                "[pipeline] Step 4.2: Selecting content declaration: "
                f"{args.content_declaration}"
            )
            publisher.set_content_declaration(args.content_declaration)
        if args.originality_declaration:
            print("[pipeline] Step 4.25: Selecting originality declaration")
            publisher.set_originality_declaration()
        if args.location:
            print(f"[pipeline] Step 4.3: Selecting location: {args.location}")
            publisher.set_location(args.location, address=args.location_address)
    ...
    if should_publish:
        ...
        try:
            publish_result = publisher._click_publish(
                post_time is not None,
                expected_title=title,
            )
            print("PUBLISH_STATUS: PUBLISHED")
            if publish_result.get("share_link"):
                print(
                    "[pipeline] Note published at: "
                    f"{publish_result['share_link']}"
                )
            elif publish_result.get("note_id"):
                print(f"[pipeline] Note ID: {publish_result['note_id']}")
        except CDPError as e:
