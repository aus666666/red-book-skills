...
XHS_NOTE_MANAGER_URL = "https://creator.xiaohongshu.com/new/note-manager?source=official"
...
XHS_NOTE_MANAGER_API_PATH = "/api/galaxy/v2/creator/note/user/posted"
XHS_NOTE_PUBLISH_API_PATH = "/web_api/sns/v2/note"
...
# Creator uploads can keep the renderer busy while the preview frame is built.
# This must be longer than a single polling tick in _wait_for_uploaded_images.
...
class XiaohongshuPublisher:
        ...
        if target_url_prefix:
            for t in pages:
                if t.get("url", "").startswith(target_url_prefix):
                    print(
                        "[cdp_publish] Reusing matching target tab: "
                        f"{t.get('url', '')}"
                    )
                    return t["webSocketDebuggerUrl"]
            raise CDPError(
                "No existing browser tab matches the required URL prefix: "
                f"{target_url_prefix}. Refusing to operate on another tab."
            )
    ...
    def connect(self, target_url_prefix: str = "", reuse_existing_tab: bool = False):
        ...
        self.ws = ws_client.connect(
            ws_url,
            # macOS may expose a system proxy even for loopback hosts. CDP is
            # always a direct connection; proxying it breaks localhost WebSocket
            # attachment (and can require optional SOCKS dependencies).
            proxy=None if _is_local_host(self.host) else True,
        )
    ...
    def _send(
        ...
        while True:
            ...
            if data.get("method"):
                ...
                if len(self._event_queue) > 2000:
                    self._event_queue = self._event_queue[-1000:]

    def _receive_cdp_message(self, timeout_seconds: float) -> dict[str, Any] | None:
        """Return a buffered CDP event or receive the next websocket message."""
        ...
        try:
            raw = self.ws.recv(timeout=max(0.1, float(timeout_seconds)))
        except TimeoutError:
            return None
        except Exception as exc:
            raise CDPError(f"CDP receive failed while waiting for an event: {exc}") from exc

        try:
            message = json.loads(raw)
        except json.JSONDecodeError as exc:
            raise CDPError(f"Received invalid CDP event JSON: {exc}") from exc
        return message if isinstance(message, dict) else None

    def _get_response_body_with_retry(
        self,
        request_id: str,
        timeout_seconds: float = 5.0,
    ) -> dict[str, Any]:
        """Read a network response body after Chrome finishes the resource."""
        ...
        while time.monotonic() < deadline:
            try:
                ...
                last_error = exc
                if "No data found for resource" not in str(exc):
                    raise
                time.sleep(0.15)
        raise last_error or CDPError(
            f"Response body unavailable for request {request_id}."
        )
    ...
    def _capture_content_data_from_page_request(
        ...
        while time.time() < deadline:
            ...
            message = self._receive_cdp_message(timeout)
            if not message:
                continue
        ...
        body_result = self._get_response_body_with_retry(target_request_id)
    ...
    def _capture_search_recommendations_via_network(
        ...
        body_result = self._get_response_body_with_retry(request_id)
        body_text = body_result.get("body", "")
        if body_result.get("base64Encoded"):
            body_text = base64.b64decode(body_text).decode("utf-8", errors="replace")
    ...
    def _get_publish_button_rect(self) -> dict[str, Any] | None:
        ...
        return self._evaluate(f"""
            (() => {{
                ...
                // Creator Center 2026-08 renders the publish controls inside an
                // opaque Web Component. Its light DOM host exposes the ready
                // state as attributes but doesn't expose the inner buttons.
                const publishHost = document.querySelector(
                    "xhs-publish-btn[is-publish='true']"
                );
                if (
                    visible(publishHost) &&
                    publishHost.getAttribute("submit-disabled") !== "true" &&
                    publishHost.getAttribute("submit-loading") !== "true"
                ) {{
                    const rect = publishHost.getBoundingClientRect();
                    // The component centers two 120px actions; submit is the
                    // right-hand action. Return its approximate clickable box.
                    return {{
    ...
    def set_content_declaration(self, declaration: str) -> dict[str, Any]:
        """Select and verify a creator-center content declaration."""
        if declaration not in CONTENT_DECLARATION_OPTIONS:
            raise CDPError(
                "Unsupported content declaration: "
                f"{declaration}. Options: {', '.join(CONTENT_DECLARATION_OPTIONS)}"
            )

        declaration_literal = json.dumps(declaration, ensure_ascii=False)

        def inspect_wrapper() -> dict[str, Any] | None:
            return self._evaluate(f"""
                (() => {{
                    const target = {declaration_literal};
                    ...
                    const wrappers = Array.from(
                        document.querySelectorAll('.d-select-wrapper')
                    ).filter(visible);
                    const selected = wrappers.find((element) =>
                        (element.innerText || element.textContent || '').trim() === target
                    );
                    const wrapper = selected || wrappers.find((element) =>
        ...
        if not wrapper:
            raise CDPError("Could not find content declaration selector.")
        if wrapper.get("selected"):
            print(f"[cdp_publish] Content declaration already selected: {declaration}")
            return {"selected": True, "declaration": declaration}

        self._click_mouse(
            wrapper["x"] + wrapper["width"] / 2,
            wrapper["y"] + wrapper["height"] / 2,
        )
        self._sleep(0.8, minimum_seconds=0.4)

        option_rect = self._evaluate(f"""
            (() => {{
                const target = {declaration_literal};
                const visible = (element) => {{
                    const rect = element.getBoundingClientRect();
                ...
                const labels = Array.from(
                    document.querySelectorAll(
                        '.declaration-drop-down .d-option-name, '
                        + '.declaration-drop-down .d-option-content'
                    )
                ).filter(visible);
                ...
                const option = label.closest('.d-option-handler') || label;
                const rect = option.getBoundingClientRect();
                return {{ x: rect.x, y: rect.y, width: rect.width, height: rect.height }};
            }})()
        """)
        if not option_rect:
            # The 2026-08 creator page can keep the declaration dropdown
            # mounted with display:none while still wiring the option rows to
            # Vue.  In that state every option has a zero-size rect, so a CDP
            # coordinate click is impossible.  Dispatch the same bubbling
            # mouse/click sequence on the exact option row as a fallback.
            dispatched = self._evaluate(f"""
                (() => {{
                    const target = {declaration_literal};
                    ...
                    const label = Array.from(document.querySelectorAll(
                        '.declaration-drop-down .d-option-name'
                    )).find((element) => textOf(element) === target);
                    if (!label) return false;
                    const gridItem = label.closest('.d-grid-item');
                    const options = label.closest('.d-options');
                    const siblings = options ? Array.from(options.children) : [];
                    const index = siblings.indexOf(gridItem);
                    const handler = index >= 0
                        ? siblings[Math.floor(index / 3) * 3]?.querySelector(
                            '.d-option-handler'
                        )
                        : null;
                    const candidates = [
                        handler,
                        label.closest('.d-option-content'),
                        label,
                        gridItem,
                    ].filter(Boolean);
            ...
            if not dispatched:
                raise CDPError(
                    f"Could not find content declaration option: {declaration}"
                )
            self._sleep(0.8, minimum_seconds=0.4)
            verified = inspect_wrapper()
            if not verified or not verified.get("selected"):
                raise CDPError(
                    f"Content declaration selection was not verified: {declaration}"
                )
            print(f"[cdp_publish] Content declaration selected: {declaration}")
            return {"selected": True, "declaration": declaration}
    ...
    def set_originality_declaration(self) -> dict[str, Any]:
        """Select and verify the creator-center originality declaration."""
        target_literal = json.dumps(ORIGINALITY_DECLARATION, ensure_ascii=False)
            ...
            return self._evaluate(f"""
                (() => {{
                    ...
                    const cards = Array.from(document.querySelectorAll('.custom-switch-card'))
                        .filter(visible)
                        .filter((element) =>
                            (element.innerText || element.textContent || '').includes(target)
                        );
                    const label = cards[0] || Array.from(document.querySelectorAll('*'))
                        .filter(visible)
                        .find((element) =>
                            (element.innerText || element.textContent || '').trim() === target
                        );
                    ...
                    const control = label.closest(
                        'label, .custom-switch-card, .d-checkbox, .d-checkbox-wrapper, '
                        + '[role="checkbox"], [class*="checkbox"]'
                    ...
                    const selected = Boolean(
                        ...
                        control.querySelector('.d-switch-box .checked, .d-switch-box .checked *') ||
                        control.querySelector(
                            'input[type="checkbox"]:checked, '
                            + '[aria-checked="true"], .checked, .is-checked'
                        )
                    );
                    ...
                    return {{
                        selected,
                        text: (control.innerText || control.textContent || '').trim(),
                        x: (control.querySelector('.d-switch') || control).getBoundingClientRect().x,
                        y: (control.querySelector('.d-switch') || control).getBoundingClientRect().y,
                        width: (control.querySelector('.d-switch') || control).getBoundingClientRect().width,
                        height: (control.querySelector('.d-switch') || control).getBoundingClientRect().height,
                    }};
                }})()
            """)
        ...
        state = inspect()
        if not state:
            raise CDPError("Could not find originality declaration control.")
        if not state.get("selected"):
            # The switch is rendered as a Vue controlled input.  On some
            # creator builds the visual switch consumes CDP pointer events
            # without forwarding the state update, while bubbling DOM events
            # on the simulator/input do.  Use that deterministic fallback.
            switched = self._evaluate("""
                (() => {
                    const card = Array.from(document.querySelectorAll('.custom-switch-card'))
                        .find((element) => (element.innerText || '').trim() === '原创声明');
                    if (!card) return false;
                    const input = card.querySelector('input[type="checkbox"]');
                    const simulator = card.querySelector('.d-switch-simulator');
                    for (const element of [simulator, input, card.querySelector('.d-switch')]) {
                        if (!element) continue;
                        element.dispatchEvent(new MouseEvent('mousedown', {
                            bubbles: true, cancelable: true, view: window,
                        }));
        ...
        # The switch being checked is not sufficient: XHS opens a mandatory
        # confirmation dialog and rejects submission until its agreement box
        # is checked and “声明原创” is pressed.
        modal_state = self._evaluate(r"""
            (() => {
                const visible = (element) => {
                    const rect = element.getBoundingClientRect();
                    const style = getComputedStyle(element);
                ...
                const modal = Array.from(document.querySelectorAll(
                    '.d-modal-content, [role="dialog"], [class*="modal-content"]'
                )).filter(visible).find((element) =>
                    textOf(element).includes('我已阅读并同意') &&
                    textOf(element).includes('原创声明须知') &&
                    textOf(element).includes('声明原创')
                );
                if (!modal) return { open: false };
                const control = Array.from(modal.querySelectorAll(
                    '.d-checkbox, label, [role="checkbox"]'
                )).find((element) =>
                    textOf(element).includes('我已阅读并同意') &&
                    textOf(element).includes('原创声明须知')
                );
                ...
                const simulator = control?.querySelector('.d-checkbox-simulator') ||
                    input?.parentElement || control;
                const checked = Boolean(
                    input?.checked || simulator?.classList.contains('checked')
                );
                if (!simulator) return { open: true, checked, checkbox: null };
                simulator.scrollIntoView({ block: 'center', inline: 'nearest' });
                const rect = simulator.getBoundingClientRect();
                return { open: true, checked,
                    checkbox: { x: rect.x, y: rect.y, width: rect.width, height: rect.height } };
            })()
        """)
        if modal_state.get("open"):
            checkbox = modal_state.get("checkbox")
            if not checkbox:
                raise CDPError("Could not find the originality declaration agreement checkbox.")
            if not modal_state.get("checked"):
                checked = self._evaluate("""
                    (() => {
                        const modal = document.querySelector('.d-modal-content');
                        const control = modal?.querySelector('.d-checkbox');
                        const input = control?.querySelector('input[type="checkbox"]');
                        for (const element of [
                            control?.querySelector('.d-checkbox-simulator'),
                            control,
                            input,
                        ]) {
                            if (!element) continue;
                ...
                self._sleep(0.5, minimum_seconds=0.25)
            confirm_rect = self._evaluate(r"""
                (() => {
                    const visible = (element) => {
                        const rect = element.getBoundingClientRect();
                        const style = getComputedStyle(element);
                    ...
                    const modal = Array.from(document.querySelectorAll(
                        ...
                        textOf(element).includes('原创声明须知') && textOf(element).includes('声明原创')
                    );
                    if (!modal) return null;
                    const button = Array.from(modal.querySelectorAll('button')).filter(visible)
                        .find((element) => textOf(element) === '声明原创');
                    if (!button || button.disabled || button.classList.contains('disabled')) return null;
            ...
            """)
            if not confirm_rect:
                raise CDPError("Originality declaration confirmation button did not become enabled.")
            confirmed = self._evaluate("""
                (() => {
                    const modal = Array.from(document.querySelectorAll(
                        '.d-modal-content, [role="dialog"], [class*="modal-content"]'
                    )).find((element) =>
                        (element.innerText || '').includes('原创声明须知') &&
                        (element.innerText || '').includes('声明原创')
                    );
                    const button = Array.from(modal?.querySelectorAll('button') || [])
                        .find((element) => (element.innerText || '').trim() === '声明原创');
                    if (!button) return false;
                    for (const element of [button, button.querySelector('.circle-button-content')]) {
                        if (!element) continue;
            ...
            if not confirmed:
                self._click_mouse(
                    confirm_rect["x"] + confirm_rect["width"] / 2,
                    confirm_rect["y"] + confirm_rect["height"] / 2,
                )
            self._sleep(0.8, minimum_seconds=0.4)

        verified = inspect()
        modal_still_open = self._evaluate("""
            (() => Array.from(document.querySelectorAll(
                ...
                return rect.width > 0 && rect.height > 0 && style.display !== 'none' &&
                    style.visibility !== 'hidden' && text.includes('原创声明须知') &&
                    text.includes('声明原创');
            }))()
        """)
        preview_verified = bool(self._evaluate("""
            (() => (document.querySelector('.permission-info-declaration')
                ?.innerText || '').includes('已声明原创'))()
        """))
        if modal_still_open and verified and verified.get("selected") and preview_verified:
            # Some creator builds apply the declaration immediately but leave
            # an already-closing modal mask mounted indefinitely.  It blocks
            # every later real mouse click even though the preview confirms
            # the declaration.  Remove only that stale overlay after both
            # independent state checks pass.
            self._evaluate("""
                (() => {
        ...
        if not verified or not verified.get("selected") or not preview_verified or modal_still_open:
            raise CDPError("Originality declaration selection was not verified.")

        print("[cdp_publish] Originality declaration selected and confirmed.")
        return {"selected": True, "declaration": ORIGINALITY_DECLARATION}

    def set_location(self, location: str, address: str | None = None) -> dict[str, Any]:
        """Search for and select an exact creator-center location option."""
        location = location.strip()
        address = (address or "").strip()
        if not location:
            raise CDPError("Location cannot be empty.")

        location_literal = json.dumps(location, ensure_ascii=False)
        address_literal = json.dumps(address, ensure_ascii=False)

        def inspect_wrapper() -> dict[str, Any] | None:
            return self._evaluate(f"""
                (() => {{
                    const target = {location_literal};
                    const expectedAddress = {address_literal};
                    const visible = (element) => {{
                        const rect = element.getBoundingClientRect();
                        const style = getComputedStyle(element);
                        return rect.width > 0 && rect.height > 0 &&
                    ...
                    const wrappers = Array.from(document.querySelectorAll(
                        '.address-card-select, .d-select-wrapper'
                    )).filter(visible);
                    const wrapper = wrappers.find((element) =>
                        element.classList.contains('address-card-select')
                    ) || wrappers.find((element) => textOf(element) === '添加地点');
                    if (!wrapper) return null;
                    const wrapperText = textOf(wrapper);
                    // The selected control displays only the place name (the
                    // address is shown in the dropdown option), so verify the
                    // exact name here.  The address was already matched before
                    // the real mouse click below.
                    const selected = wrapperText.includes(target);
                    const control = wrapper.querySelector('.d-select-main') || wrapper;
                    control.scrollIntoView({{ block: 'center', inline: 'nearest' }});
                    const rect = control.getBoundingClientRect();
                    return {{
                        selected: Boolean(selected), text: wrapperText,
        ...
        wrapper = inspect_wrapper()
        if not wrapper:
            raise CDPError("Could not find location selector.")
        if wrapper.get("selected"):
            print(f"[cdp_publish] Location already selected: {location}")
            return {"selected": True, "location": location, "address": address}
        ...
        search_input = self._evaluate("""
            (() => {
                const wrapper = document.querySelector('.address-card-select');
                const input = wrapper?.querySelector(
                    '.d-select-input-filter input, input[type="text"]'
                );
        ...
        """)
        if not search_input:
            raise CDPError("Could not find the location search input.")
        self._send("Input.insertText", {"text": location.split("(", 1)[0]})
        self._sleep(1.0, minimum_seconds=0.5)

        option_rect = None
        option_deadline = time.monotonic() + 8.0
        while time.monotonic() < option_deadline and not option_rect:
            option_rect = self._evaluate(f"""
                (() => {{
                    const target = {location_literal};
                    const expectedAddress = {address_literal};
                    ...
                    const option = Array.from(document.querySelectorAll(
                        ...
                        const name = textOf(element.querySelector('.option-name'));
                        const subname = textOf(element.querySelector('.option-subname'));
                        return name === target &&
                            (!expectedAddress || subname === expectedAddress);
                    }});
                    ...
                    return {{
                        x: rect.x, y: rect.y, width: rect.width, height: rect.height,
                        actualAddress: textOf(option.querySelector('.option-subname')),
                    }};
                }})()
            """)
            if not option_rect:
                time.sleep(0.25)
        if not option_rect:
            raise CDPError(f"Could not find exact location option: {location}")
        ...
        verified = inspect_wrapper()
        if not verified or not verified.get("selected"):
            # Location results occasionally ignore coordinate clicks even
            # though the exact visible row was found.  Replay the event
            # sequence on the exact name/address option before failing.
            dispatched = self._evaluate(f"""
                (() => {{
                    const target = {location_literal};
                    const expectedAddress = {address_literal};
                    ...
                    const option = Array.from(document.querySelectorAll(
                        ...
                        textOf(element.querySelector('.option-name')) === target &&
                        (!expectedAddress ||
                            textOf(element.querySelector('.option-subname')) === expectedAddress)
                    );
                    if (!option) return false;
                    for (const element of [option.querySelector('.option-name'), option]) {{
                        if (!element) continue;
                        element.dispatchEvent(new MouseEvent('mousedown', {{
                            bubbles: true, cancelable: true, view: window,
                        }}));
            ...
            if dispatched:
                ...
                verified = inspect_wrapper()
        if not verified or not verified.get("selected"):
            raise CDPError(f"Location selection was not verified: {location}")

        print(
            f"[cdp_publish] Location selected: {location}"
            + (f" ({address})" if address else "")
        )
        return {"selected": True, "location": location, "address": address}

    def _find_note_title_in_manager(self, title: str) -> bool:
        """Return whether an exact visible note title exists on the current page."""
        title_literal = json.dumps(title, ensure_ascii=False)
        return bool(self._evaluate(f"""
            (() => {{
                const target = {title_literal};
                const visible = (element) => {{
                    const rect = element.getBoundingClientRect();
                    const style = getComputedStyle(element);
                    return rect.width > 0 && rect.height > 0 &&
                ...
                return Array.from(document.querySelectorAll('*')).some((element) =>
                    visible(element) && element.children.length === 0 &&
                    (element.innerText || element.textContent || '').trim() === target
                );
            }})()
        """))

    def _capture_note_manager_notes(self, retries: int = 3) -> list[dict[str, Any]]:
        """Capture Creator Center's own note-manager response through CDP.

        The manager's first request can intermittently receive HTTP 406 while
        the browser completes its dynamic security-profile handshake.  Calling
        the endpoint separately loses those page-generated headers.  Repeating
        the normal page navigation and reading its network response preserves
        the user's existing browser session without inspecting or persisting
        ...
        for attempt in range(max(1, retries)):
            self._send("Page.navigate", {"url": XHS_NOTE_MANAGER_URL})
            request_urls: dict[str, str] = {}
            deadline = time.monotonic() + 12.0

            while time.monotonic() < deadline:
                message = self._receive_cdp_message(
                ...
                if method == "Network.requestWillBeSent":
                    ...
                    if isinstance(request_id, str):
                        request_urls[request_id] = str(request_url)
                    continue
                ...
                if not isinstance(request_id, str):
                    continue
                if XHS_NOTE_MANAGER_API_PATH not in request_urls.get(request_id, ""):
                    continue
                ...
                try:
                    ...
                    raise CDPError(
                        "Could not decode note-manager response JSON: "
                        f"{exc}; preview={body_text[:300]}"
                    ) from exc
                ...
                raise CDPError("Unexpected note-manager response structure.")

            if attempt + 1 < max(1, retries):
                self._sleep(1.2, minimum_seconds=0.5)

        raise CDPError(
            "Creator Center note-manager API did not yield a usable response "
            f"after {max(1, retries)} page-native attempt(s); last HTTP status={last_status}."
        )

    @staticmethod
    def _manager_note_status(note: dict[str, Any]) -> str:
        """Map current note-manager tab status values to visible Chinese labels."""
    ...
    def verify_note_submission(
        self,
        title: str,
        timeout_seconds: float = 40.0,
    ) -> dict[str, Any]:
        """Verify an exact title in note manager and resolve its status tab."""
        title = title.strip()
        if not title:
            raise CDPError("Note title cannot be empty for verification.")

        deadline = time.time() + max(5.0, timeout_seconds)
        status_tabs = ("审核中", "已发布", "未通过")
        while time.time() < deadline:
            self._navigate(XHS_NOTE_MANAGER_URL)
            ...
            for status in status_tabs:
                ...
                self._sleep(1.8, minimum_seconds=0.8)
                if self._find_note_title_in_manager(title):
                    result = {
                        "found": True,
                        "title": title,
                        "status": status,
                        "manager_url": XHS_NOTE_MANAGER_URL,
                    }
                    print(
                        "[cdp_publish] Note verification succeeded: "
            ...
            # DOM rendering can remain on the empty state when the manager's
            # first request hit 406.  The browser-native API response is the
            # authoritative fallback and does not require reconstructing its
            # anti-bot headers in a separate fetch.
            try:
                notes = self._capture_note_manager_notes(retries=2)
            except CDPError as exc:
                print(f"[cdp_publish] Note-manager API fallback unavailable: {exc}")
            else:
                for note in notes:
                    note_title = str(
                    ...
                    result = {
                        "found": True,
                        "title": title,
                        "status": self._manager_note_status(note),
                        "note_id": note.get("id"),
                        "manager_url": XHS_NOTE_MANAGER_URL,
                        "verification_source": "manager_api_capture",
                    }
                    print(
                        "[cdp_publish] Note verification succeeded via "
                        f"manager API capture: title={title}, status={result['status']}"
                    )
                    return result
    ...
    def _current_publish_form(self) -> dict[str, Any]:
        """Inspect the active creator form before an external submit."""
    ...
    def _get_accessible_button_rect(self, button_name: str) -> dict[str, Any] | None:
        """Locate one visible named button through Chromium's accessibility tree."""
        ...
        for node in tree.get("nodes", []):
            ...
            if role != "button" or name != button_name or not backend_node_id:
                continue
            try:
                model = self._send(
            ...
            if model.get("width", 0) <= 0 or model.get("height", 0) <= 0:
                continue
            if len(border) != 8:
                continue
            return {
    ...
    def _activate_publish_button(self, button: dict[str, Any]) -> str:
        """Activate the current publish control through its supported event path.

        Creator Center 2026-08 renders the action inside a closed shadow root.
        The custom element's native button calls ``_onPublish``, which emits a
        composed ``publish`` event from the host; trusted mouse and keyboard
        input can focus the inner button without invoking that callback.  Emit
        the same public host event when the ready web component is present.
        Legacy pages still use trusted accessibility-tree input as a fallback.
        """
        dispatched = self._evaluate("""
            (() => {
                const host = document.querySelector(
                    'xhs-publish-btn[is-publish="true"]'
                );
                ...
                if (host.getAttribute('submit-disabled') === 'true' ||
                    host.getAttribute('submit-loading') === 'true') return false;
        ...
        """)
        if dispatched:
            return "web_component_publish_event"

        backend_node_id = button.get("backendDOMNodeId")
        if backend_node_id:
            try:
                self._send("DOM.focus", {"backendNodeId": backend_node_id})
                ...
                return "focused_enter"
            except CDPError as error:
                print(
                    "[cdp_publish] Warning: closed-shadow button keyboard "
                    f"activation failed ({error}); falling back to mouse."
                )

        self._click_mouse(float(button["x"]), float(button["y"]))
        return "mouse_fallback"

    def _capture_publish_validation_state(self) -> dict[str, Any]:
        """Capture visible platform validation feedback before navigation."""
        state = self._evaluate(r"""
            (() => {
                ...
                for (const element of document.querySelectorAll(selectors.join(','))) {
                    ...
                    if (feedback.length >= 20) break;
                }
                const invalidFields = Array.from(document.querySelectorAll(
                ...
                return {
                    ...
                    feedback,
                    invalidFields,
                    publishHost: host ? Object.fromEntries(
                        Array.from(host.attributes).map((attribute) => [
    ...
    def _wait_for_publish_response(
        self,
        timeout_seconds: float = 25.0,
    ) -> dict[str, Any]:
        """Wait for and validate the creator-center note submission response."""
        ...
        while time.monotonic() < deadline:
            ...
            if method == "Network.requestWillBeSent":
                ...
                if (
                    ...
                    and XHS_NOTE_PUBLISH_API_PATH in request_url
                ):
            ...
            success = bool(
                ...
                and (payload.get("success") is True or payload.get("result") == 0)
            )
            if not success:
                raise CDPError(
                    "Xiaohongshu rejected the publish request: "
                    f"HTTP {status}, response={body[:500] or '<empty>'}"
                )
        ...
        # A successful submit may navigate the creator tab and detach the old
        # CDP target before its Network.responseReceived event reaches us.
        # Verify the exact title in note manager before classifying this as a
        # rejected submission.  This avoids both false failures and blind
        # duplicate retries.
        raise CDPError(
            "Publish click produced no Xiaohongshu note submission response. "
            "The platform did not accept the form; do not retry blindly."
        )
    ...
    def _click_publish(
        ...
        """Click once and require a successful Xiaohongshu submit response."""
        print("[cdp_publish] Clicking publish button...")
        self._sleep(ACTION_INTERVAL, minimum_seconds=0.25)
        form = self._current_publish_form()
        if not str(form.get("url") or "").startswith(
            "https://creator.xiaohongshu.com/publish"
        ):
            raise CDPError(
                "Refusing to click publish outside the creator publish page: "
                f"{form.get('url') or '<unknown>'}"
            )
        actual_title = str(form.get("title") or "").strip()
        if not actual_title:
            raise CDPError("Refusing to publish: the active form has no title.")
        if expected_title and actual_title != expected_title.strip():
            raise CDPError(
                "Refusing to publish the wrong form: "
                f"expected title {expected_title!r}, found {actual_title!r}."
            )
        host = form.get("host") or {}
        if host.get("submit-disabled") == "true":
            raise CDPError("Refusing to publish: submit-disabled=true.")
        if host.get("submit-loading") == "true":
            raise CDPError("Refusing to publish: submit-loading=true.")

        self._wait_for_publish_button_ready(timeout_seconds=20.0)
        rect = self._get_accessible_button_rect(
            SELECTORS["schedule_publish_button_text"]
            if scheduled
            else SELECTORS["publish_button_text"]
        )
        if not rect:
            raise CDPError(
                "Could not find the real publish button in the accessibility tree."
            )

        self._send("Network.enable", {"maxPostDataSize": 1048576})
        cx = rect["x"]
        cy = rect["y"]
        print(f"[cdp_publish] Activating publish button at ({cx:.0f}, {cy:.0f})...")
        activation_mode = self._activate_publish_button(rect)
        print(
            "[cdp_publish] Publish button activated "
            f"(mode={activation_mode})."
        )
        try:
            result = self._wait_for_publish_response()
        except CDPError as error:
            if "no Xiaohongshu note submission response" not in str(error):
                raise
            validation_state = self._capture_publish_validation_state()
            print("PUBLISH_VALIDATION_STATE:")
            print(json.dumps(validation_state, ensure_ascii=False, indent=2))
            title_to_verify = expected_title or actual_title
            verified = self.verify_note_submission(
                title=title_to_verify,
                timeout_seconds=30.0,
            )
            if not verified.get("found"):
                feedback = validation_state.get("feedback") or []
                suffix = (
                    f" Visible platform feedback: {' | '.join(feedback[:5])}"
                    if feedback else ""
                )
                raise CDPError(f"{error}{suffix}")
            result = {
                "request_id": None,
                "status": 200,
                "note_id": None,
                ...
                "capture_mode": "manager_fallback",
            }
        print(
            "[cdp_publish] Publish request accepted by Xiaohongshu"
            + (f" (note_id={result['note_id']})." if result.get("note_id") else ".")
        )
        return result
...
def main():
    ...
    # click-publish - just click the publish button on a matching filled page
    p_click_publish = sub.add_parser(
        "click-publish",
        help="Click publish on an already-filled page with the exact title",
    )
    p_click_publish.add_argument(
        "--title",
        required=True,
        help="Exact title required on the active publish form",
    )

    p_verify_note = sub.add_parser(
        "verify-note",
        aliases=["verify_note"],
        help="Verify an exact note title in creator note manager",
    )
    p_verify_note.add_argument("--title", required=True, help="Exact note title")
    p_verify_note.add_argument(
        "--timeout-seconds",
        type=float,
        default=40.0,
        help="Maximum wait for the title to appear (default: 40)",
    )
    ...
    try:
        if args.command == "check-login":
            ...
            if args.command == "publish":
                publisher._click_publish(expected_title=args.title)
        ...
        elif args.command in ("verify-note", "verify_note"):
            publisher.connect(reuse_existing_tab=reuse_existing_tab)
            result = publisher.verify_note_submission(
                title=args.title,
                timeout_seconds=args.timeout_seconds,
            )
            print("VERIFY_NOTE_RESULT:")
            print(json.dumps(result, ensure_ascii=False, indent=2))
            sys.exit(0 if result.get("found") else 4)
