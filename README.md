...
- **新版发布控件兼容**：支持 2026 年 8 月创作者中心的 `xhs-publish-btn` Web Component，通过组件公开的 composed `publish` 事件进入页面提交逻辑
- **内容类型声明**：发布前可选择“笔记含AI合成内容”等平台声明并校验选中状态
- **提交状态核验**：可按精确标题在笔记管理页核验“审核中 / 已发布 / 未通过”状态
...
建议为项目使用隔离环境，并始终用同一个解释器安装依赖和运行脚本：

```bash
python3 -m venv .venv
.venv/bin/python -m pip install -r requirements.txt
.venv/bin/python scripts/cdp_publish.py check-login
```

若出现 `ModuleNotFoundError: websockets.sync`，说明当前解释器没有加载 `requirements.txt` 要求的新版 `websockets`。macOS 的 `/usr/bin/python3` 尤其容易与项目虚拟环境混用；先执行 `python -c 'import sys; print(sys.executable)'` 核对路径，再改用已安装依赖的 `.venv/bin/python`。该错误发生在 CDP 连接之前，与账号登录态或页面选择器无关。

笔记管理页偶发显示“全部 0”时，不要据此重新发布。页面的 `/api/galaxy/v2/creator/note/user/posted` 首请求可能短暂返回 406；`verify-note` 会重复正常页面导航并捕获浏览器自身的成功响应，避免单独重建带动态安全头的请求或记录任何会话凭据。
...
选项:
  ...
  --content-declaration TEXT 内容类型声明
  --location TEXT        发布地点的精确名称
  --location-address TEXT 要求地点匹配的精确地址
```
...
说明：发布成功必须同时满足：当前页是发布页、标题精确匹配、真实发布按钮可用，并收到 `POST /web_api/sns/v2/note` 的成功响应。仅发出鼠标点击不会返回 `PUBLISHED`。
...
# 点击已填充发布页的按钮；标题不匹配时拒绝点击
python scripts/cdp_publish.py click-publish --title "文章标题"
